//Odd Lists
/*
Write a function that returns an Array that contains every other element of an 
Array that is passed in as an argument. The values in the returned list should
be those values that are in the 1st, 3rd, 5th, and so on elements of the argument
Array.
*/

let oddities = (ary) => {
  let resultArray = [];

  for (let i = 0; i < ary.length; i += 2) {
    resultArray.push(ary[i]);
  }

  return resultArray;
}

let evenities = (ary) => {
  let resultArray = [];

  for (let i = 1; i < ary.length; i += 2) {
    resultArray.push(ary[i]);
  }

  return resultArray;
}

let newOddities = (ary) => {
  let resultArray = ary.filter((num, index) => {
    return index % 2 === 0;
  })

  return resultArray;
}

console.log(newOddities([2, 3, 4, 5, 6])); // logs [2, 4, 6]
console.log(newOddities([1, 2, 3, 4, 5, 6])); // logs [1, 3, 5]
console.log(newOddities(["abc", "def"])); // logs ['abc']
console.log(newOddities([123])); // logs [123]
console.log(newOddities([])); // logs []